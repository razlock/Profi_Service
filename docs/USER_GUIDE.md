...
Nika CRM — веб-система для сервисного центра: заявки на ремонт, клиенты и устройства, склад и закупки, касса, магазин, зарплата, отчёты, внутренний чат и личный кабинет клиента. Как пользоваться экранами — в этом руководстве; короткий обзор ядра — [серия 1 в блоге](/blog/core-orders-warehouse-cash).
...
**Windows Offline:** после сохранения почты перезапустите службу ярлыком **«Nika CRM — Перезапуск службы»** на рабочем столе — иначе процесс может ещё держать старые `MAIL_*` из `.env`.

#### Главная ошибка: «От кого» ≠ логин

Логин и адрес отправителя — **разные поля**. В демо-базе часто лежит:

`Nika CRM Demo <noreply@example.com>`
...
4. После установки откройте Nika CRM из меню «Пуск» или ярлыка. Чтобы **закрепить на панели задач**: ПКМ по иконке на панели → **«Закрепить на панели задач»**.
...
**Windows: сохранили SMTP, тест всё равно со старыми настройками.** Перезапустите службу ярлыком **«Nika CRM — Перезапуск службы»**, затем повторите тест.
